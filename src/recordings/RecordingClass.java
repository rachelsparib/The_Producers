...
public class RecordingClass implements Recording {
	 ...
	 * 
	 * @param users
	 *            collaborators participating in the recording.
	 * @param date
	 *            initial date of the recording.
	 * @param duration
	 *            duration of the recording.
				...
				if (this.getStartDate() == null) {
					...
					if (this.getEndDate() == null) {
						...
						if (other.getStartDate().isAfter(this.getStartDate())
								&& other.getStartDate().isBefore(this.getEndDate())) { // time
																						// collision
							Iterator<User> it = other.getCollabs().iterator(); // iterator
																				// of
																				// collabs
																				// of
																				// the
							...
							while (it.hasNext()) {
								User u = it.next();
								if (collabs.searchIndexOf(u) >= 0) // share at
																	// least one
																	// collaborator
									return true;
							}
							// collabs not shared
							if (!local.getName().equals(other.getLocal().getName())) // time
																						// collision,
																						// local
																						// not
																						// shared
							...
							if (other.getStartDate().isEqual(this.getStartDate())
									&& local.getName().equals(other.getLocal().getName()))
								return true;
						}
					}
				}
	...
	/**
	 * Return the next available schedule for the collaborator <code>u</code> or
	 * null if there is no appointments.
	 * 
	 * @param u
	 *            a collaborator.
	 * @return the next available schedule for the collaborator <code>u</code>
	 *         or null if there is no appointments.
	 */
	...
	// @Override
	// public boolean isCollabAvailable(User u, LocalDateTime start,
	// LocalDateTime end) {
	// return false;
	// }
	...
	public String toString() {
		...
		User producer = getProducer();
		User director = getDirector();
