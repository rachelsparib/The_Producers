...
public class Main {
						...
						if (!p.hasUser(directorName)) // level 5
							...
							if (!p.hasUser(technicianName)) // level 6
								...
								if (p.hasUserCollection(collabsName)) // level 7
									...
									collabsName.insertLast(directorName); // to
																			// check
																			// blacklist
																			// conflicts
																			// with
									...
									if (p.hasBlacklistConflict(collabsName) && !p.hasRecording(localname, start)) { // level
																													// 8
										...
										if (p.hasRecordingConflict(localname, start)
												&& !p.hasProducerPriority(localname, start)) { // level
																								// 9
											System.out.println(MessagesEnum.RECORD_CONFLICT);
										} else {
											if (p.hasRecordingConflict(localname, start)
													&& p.hasProducerPriority(localname, start)
													&& !p.hasRecording(localname, start)) { // level
																							// 10
	...
	private static void places(Scanner in, Production p) {
		...
		if (l == null) {
			...
			Iterator<Recording> it = p.getRecCollectionByStatus(RecordingStatusEnum.SCHEDULED);
